...
static const struct twim twims[TWIM_COUNT] = {
    {
        .nrf_twim = NRF_TWIM20_S,
        .irqn = TWIM20_IRQn,
        .isr = twim0_irq_handler,
    },
    {
        .nrf_twim = NRF_TWIM21_S,
        .irqn = TWIM21_IRQn,
        .isr = twim1_irq_handler,
    },
    {
        .nrf_twim = NRF_TWIM22_S,
        .irqn = TWIM22_IRQn,
...
static int
bus_i2c_nrf54lxx_init_node(struct bus_dev *bdev, struct bus_node *bnode, void *arg)
{
...
static int
bus_i2c_nrf54lxx_enable(struct bus_dev *bdev)
{
...
static int
bus_i2c_nrf54lxx_configure_controller(struct bus_i2c_dev *dev, uint8_t address, uint16_t freq)
{
    NRF_TWIM_Type *nrf_twim;
    int rc = 0;
...
static int
bus_i2c_nrf54lxx_configure(struct bus_dev *bdev, struct bus_node *bnode)
{
    struct bus_i2c_dev *dev = (struct bus_i2c_dev *)bdev;
    struct bus_i2c_node *node = (struct bus_i2c_node *)bnode;
    ...
    return bus_i2c_nrf54lxx_configure_controller(dev, node->addr, node->freq);
}

static int
bus_i2c_nrf54lxx_read(struct bus_dev *bdev, struct bus_node *bnode,
                     ...
                     uint16_t flags)
{
    ...
    nrf_twim->DMA.RX.PTR = (uint32_t)buf;
    nrf_twim->DMA.RX.MAXCNT = length;

    nrf_twim->EVENTS_STOPPED = 0;
    nrf_twim->EVENTS_ERROR = 0;
    nrf_twim->EVENTS_SUSPENDED = 0;
    nrf_twim->EVENTS_LASTRX = 0;
    nrf_twim->EVENTS_DMA.RX.END = 0;
    nrf_twim->EVENTS_DMA.RX.BUSERROR = 0;
    nrf_twim->EVENTS_DMA.RX.MATCH[0] = 0;
    nrf_twim->EVENTS_DMA.RX.MATCH[1] = 0;
    nrf_twim->EVENTS_DMA.RX.MATCH[2] = 0;
    nrf_twim->EVENTS_DMA.RX.MATCH[3] = 0;
    nrf_twim->EVENTS_DMA.RX.READY = 0;
...
static int
bus_i2c_nrf54lxx_write(struct bus_dev *bdev, struct bus_node *bnode,
                      ...
                      uint16_t flags)
{
    ...
    nrf_twim->DMA.TX.MAXCNT = length;
    nrf_twim->INTEN = 0;

    nrf_twim->DMA.TX.PTR = (uint32_t)buf;

    nrf_twim->EVENTS_ERROR = 0;
    nrf_twim->EVENTS_STOPPED = 0;
    nrf_twim->EVENTS_SUSPENDED = 0;
    nrf_twim->EVENTS_LASTTX = 0;
    nrf_twim->EVENTS_DMA.TX.READY = 0;
    nrf_twim->EVENTS_DMA.TX.BUSERROR = 0;
    nrf_twim->EVENTS_DMA.TX.END = 0;
...
static int
bus_i2c_nrf54lxx_disable(struct bus_dev *bdev)
{
    struct bus_i2c_dev *dev = (struct bus_i2c_dev *)bdev;
    NRF_TWIM_Type *nrf_twim;
...
static int
bus_i2c_nrf54lxx_probe(struct bus_i2c_dev *dev, uint16_t address, os_time_t timeout)
{
    struct twim_dev_data *dd;
    NRF_TWIM_Type *nrf_twim;
    int rc;
    ...
    if (rc == 0) {
        rc = bus_i2c_nrf54lxx_configure_controller(dev, address, 100);
        assert(rc == 0);
        nrf_twim = twims[dev->cfg.i2c_num].nrf_twim;
        dd = &twim_devs_data[dev->cfg.i2c_num];

        nrf_twim->DMA.TX.MAXCNT = 0;
...
static const struct i2c_dev_ops bus_i2c_nrf54lxx_ops = {
    .bus_ops = {
        .init_node = bus_i2c_nrf54lxx_init_node,
        .enable = bus_i2c_nrf54lxx_enable,
        .configure = bus_i2c_nrf54lxx_configure,
        .read = bus_i2c_nrf54lxx_read,
        .write = bus_i2c_nrf54lxx_write,
        .disable = bus_i2c_nrf54lxx_disable,
    },
    .probe = bus_i2c_nrf54lxx_probe,
};

#include <hal/hal_gpio.h>

int
bus_i2c_nrf54lxx_dev_init_func(struct os_dev *odev, void *arg)
{
     ...
     */
    nrf_gpio = NRF_P1;
    nrf_gpio->PIN_CNF[8] = TWIM_GPIO_PIN_CNF;
    nrf_gpio = NRF_P1;
    nrf_gpio->PIN_CNF[9] = TWIM_GPIO_PIN_CNF;
    ...
    rc = bus_dev_init_func(odev, (void*)&bus_i2c_nrf54lxx_ops);
    assert(rc == 0);
