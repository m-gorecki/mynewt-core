...
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__ && MYNEWT_VAL_CHOICE(ILI9341_MCU_INTERFACE, 4_wire_spi)
...
#ifndef ILI9341_CS_PIN_ACTIVE
#define ILI9341_CS_PIN_ACTIVE() ili9341_cs_pin_set(0)
#endif
#ifndef ILI9341_CS_PIN_INACTIVE
#define ILI9341_CS_PIN_INACTIVE() ili9341_cs_pin_set(1)
#endif

#ifndef ILI9341_DC_PIN_DATA
#define ILI9341_DC_PIN_DATA() ili9341_dc_pin_set(1)
#endif

#ifndef ILI9341_DC_PIN_COMMAND
#define ILI9341_DC_PIN_COMMAND() ili9341_dc_pin_set(0)
#endif

#ifndef ILI9341_RESET_PIN_INACTIVE
#define ILI9341_RESET_PIN_INACTIVE() ili9341_reset_pin_set(1)
#endif

#ifndef ILI9341_RESET_PIN_ACTIVE
#define ILI9341_RESET_PIN_ACTIVE() ili9341_reset_pin_set(0)
#endif

void
ili9341_cs_pin_set(int val)
{
    hal_gpio_write(MYNEWT_VAL(ILI9341_CS_PIN), val);
}

void
ili9341_dc_pin_set(int val)
{
    hal_gpio_write(MYNEWT_VAL(ILI9341_DC_PIN), val);
}

void
ili9341_reset_pin_set(int val)
{
    if (MYNEWT_VAL(ILI9341_RESET_PIN) >= 0) {
        hal_gpio_write(MYNEWT_VAL(ILI9341_RESET_PIN), val);
    }
}
#if MYNEWT_VAL_CHOICE(ILI9341_MCU_INTERFACE, 4_wire_spi)
static struct bus_spi_node lcd;
static struct bus_spi_node_cfg lcd_spi_cfg = {
    .node_cfg.bus_name = MYNEWT_VAL(ILI9341_SPI_DEV_NAME),
    .pin_cs = MYNEWT_VAL(ILI9341_CS_PIN),
    .mode = BUS_SPI_MODE_0,
    .data_order = HAL_SPI_MSB_FIRST,
    .freq = MYNEWT_VAL(ILI9341_SPI_FREQ),
};
static struct os_dev *lcd_dev;

void
ili9341_spi_write_byte(uint8_t val)
{
    bus_node_simple_write(lcd_dev, &val, 1);
}

void
ili9341_spi_write_bytes(const uint8_t *buf, int count)
{
    bus_node_simple_write(lcd_dev, buf, count);
}

#ifndef ILI9341_WRITE_BYTES
#define ILI9341_WRITE_BYTES(buf, count) ili9341_spi_write_bytes(buf, count)
#endif

#ifndef ILI9341_WRITE_BYTE
#define ILI9341_WRITE_BYTE(buf) ili9341_spi_write_byte(buf)
#endif

#elif MYNEWT_VAL_CHOICE(ILI9341_MCU_INTERFACE, 8080_8_bit)
#endif

static inline void ili9341_cmd(uint8_t data);
static inline void ili9341_data(const uint8_t *data, uint16_t len);

static void
ili9341_rotate(lv_disp_rot_t rot)
{
    uint8_t color_order = MYNEWT_VAL(ILI9341_USE_BGR) ? MADCTL_BGR : MADCTL_RGB;

    ili9341_cmd(ILI9341_MADCTL);

    switch (rot) {
    case LV_DISP_ROT_270:
        color_order |= MADCTL_MV;
        break;
    case LV_DISP_ROT_180:
        color_order |= MADCTL_MY;
        break;
    case LV_DISP_ROT_90:
        color_order |= MADCTL_MX | MADCTL_MY | MADCTL_MV;
        break;
    case LV_DISP_ROT_NONE:
        color_order |= MADCTL_MX;
        break;
    }
    ili9341_data(&color_order, 1);
}
...
ili9341_init(lv_disp_drv_t *driver)
{
    ...
    /* hardware reset */
    ILI9341_CS_PIN_INACTIVE();
    ILI9341_DC_PIN_DATA();
    ILI9341_RESET_PIN_ACTIVE();
    LV_DRV_DELAY_US(50);
    ILI9341_RESET_PIN_INACTIVE();
    LV_DRV_DELAY_MS(5);

    /* software reset */
    ili9341_cmd(ILI9341_SWRESET);
    LV_DRV_DELAY_MS(5);
    ili9341_cmd(ILI9341_DISPOFF);

    /* startup sequence */
    ili9341_cmd(ILI9341_PWCTRLB);
    data[0] = 0x00;
    data[1] = 0x83;
    data[2] = 0x30;
    ili9341_data(data, 3);

    ili9341_cmd(ILI9341_PWSEQCTRL);
    data[0] = 0x64;
    data[1] = 0x03;
    data[2] = 0x12;
    data[3] = 0x81;
    ...
    ili9341_cmd(ILI9341_TIMECTRLA_INT);
    data[0] = 0x85;
    data[1] = 0x01;
    data[2] = 0x79;
    ili9341_data(data, 3);

    ili9341_cmd(ILI9341_PWCTRLA);
    data[0] = 0x39;
    data[1] = 0x2c;
    data[2] = 0x00;
    data[3] = 0x34;
    ...
    ili9341_cmd(ILI9341_PUMPRATIO);
    data[0] = 0x20;
    ili9341_data(data, 1);

    ili9341_cmd(ILI9341_TIMECTRLB);
    data[0] = 0x00;
    data[1] = 0x00;
    ili9341_data(data, 2);

    /* power control */
    ili9341_cmd(ILI9341_PWCTRL1);
    data[0] = 0x26;
    ili9341_data(data, 1);

    ili9341_cmd(ILI9341_PWCTRL2);
    data[0] = 0x11;
    ili9341_data(data, 1);

    /* VCOM */
    ili9341_cmd(ILI9341_VMCTRL1);
    data[0] = 0x35;
    data[1] = 0x3e;
    ili9341_data(data, 2);

    ili9341_cmd(ILI9341_VMCTRL2);
    data[0] = 0xbe;
    ili9341_data(data, 1);

    /* set orientation */
    ili9341_rotate(LV_DISP_ROT_NONE);

    /* 16 bit pixel */
    ili9341_cmd(ILI9341_PIXSET);
    data[0] = 0x55;
    ili9341_data(data, 1);

    /* frame rate */
    ili9341_cmd(ILI9341_FRMCTR1);
    data[0] = 0x00;
    data[1] = 0x1b;
    ili9341_data(data, 2);
...
#if ILI9341_GAMMA
    /* gamma curve set */
    ili9341_cmd(ILI9341_GAMSET);
    data[0] = 0x01;
    ili9341_data(data, 1);

    /* positive gamma correction */
    ili9341_cmd(ILI9341_PGAMCTRL);
    data[0] = 0x1f;
    data[1] = 0x1a;
    data[2] = 0x18;
    data[3] = 0x0a;
    ...
    data[13] = 0x05;
    data[14] = 0x00;
    ili9341_data(data, 15);

    /* negative gamma correction */
    ili9341_cmd(ILI9341_NGAMCTRL);
    data[0] = 0x00;
    data[1] = 0x25;
    data[2] = 0x27;
    data[3] = 0x05;
    ...
    /* window horizontal */
    ili9341_cmd(ILI9341_CASET);
    data[0] = 0;
    data[1] = 0;
    data[2] = (driver->hor_res - 1) >> 8;
    data[3] = (driver->hor_res - 1);
    ili9341_data(data, 4);

    /* window vertical */
    ili9341_cmd(ILI9341_PASET);
    data[0] = 0;
    data[1] = 0;
    data[2] = (uint8_t)((driver->ver_res - 1) >> 8);
    data[3] = (uint8_t)(driver->ver_res - 1);
    ili9341_data(data, 4);

    ili9341_cmd(ILI9341_RAMWR);

#if ILI9341_TEARING
    /* tearing effect off */
    ili9341_cmd(ILI9341_TEOFF);

    /* tearing effect on */
    ili9341_cmd(ILI9341_TEON);
#endif

#if MYNEWT_VAL_CHOICE(ILI9341_MCU_INTERFACE, 8080_8_bit)
    ili9341_cmd(ILI9341_IFCTL);
    data[0] = 1;
    data[1] = 0;
    data[2] = 1 << 5;
    ili9341_data(data, 3);
    ...
    /* entry mode set */
    ili9341_cmd(ILI9341_ETMOD);
    data[0] = 0x07;
    ili9341_data(data, 1);

    /* display function control */
    ili9341_cmd(ILI9341_DISCTRL);
    data[0] = 0x0a;
    data[1] = 0x82;
    data[2] = 0x27;
    data[3] = 0x00;
    ...
    /* exit sleep mode */
    ili9341_cmd(ILI9341_SLPOUT);

    LV_DRV_DELAY_MS(100);

    /* display on */
    ili9341_cmd(ILI9341_DISPON);

    LV_DRV_DELAY_MS(20);
}

lv_color16_t pride[240];
bool pride_init;
int pride_shift;
uint8_t pride_mask = 0xFF;
bool hack = false;
...
ili9341_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    if (!pride_init && hack) {
        for (i = 0; i < 240; ++i) {
            pride[i].full = (pride_mask & i) << pride_shift;
        }
        pride_init = true;
    }
    ...
    for (y = act_y1; y <= act_y2; y++) {
        if (act_y1 == 0 && hack) {
            ili9341_data((uint8_t *)pride, w * 2);
        } else {
            ili9341_data((uint8_t *)color_p, w * 2);
        }
        color_p += w;
    }
...
{
    ILI9341_DC_PIN_COMMAND();
    ILI9341_CS_PIN_ACTIVE();
    ILI9341_WRITE_BYTE(cmd);
    ILI9341_CS_PIN_INACTIVE();
}
 ...
 */
static inline void
ili9341_data(const uint8_t *data, uint16_t len)
{
    ILI9341_CS_PIN_ACTIVE();
    ILI9341_DC_PIN_DATA();
    ILI9341_WRITE_BYTES(data, len);
    ILI9341_CS_PIN_INACTIVE();
}
...
mynewt_lv_drv_init(lv_disp_drv_t *driver)
{
    ...
    rc = bus_spi_node_create("ili9341", &lcd, &lcd_spi_cfg, NULL);
    assert(rc == 0);
    lcd_dev = os_dev_open("ili9341", 0, NULL);
#elif MYNEWT_VAL_CHOICE(ILI9341_MCU_INTERFACE, 8080_8_bit)
#else
#error Unsupported/unset ILI9341 MCU interface
#endif
